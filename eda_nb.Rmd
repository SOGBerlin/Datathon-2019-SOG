...
```{r}
# Data Set 1
haupttabelle_raw_df <- read_csv(file = "data/Haupttabelle_ueberarbeitet_20180424 - Tabelle1.csv")

# Data Set 2
sheets <- excel_sheets(path = "data/Survey Responses SOG.xlsx")

survey_responses_raw <- sheets %>%
  keep(str_detect(string = ., pattern = "r")) %>% 
  map(.f = function(sheet) {
    read_excel(path = "data/Survey Responses SOG.xlsx", sheet = sheet)
  }
)
```
...
haupttabelle_df <- haupttabelle_raw_df %>% 
  ...
  mutate(Abschlussnote = as_factor(Abschlussnote, level = c("d", "s", "fail", "ausstehend")))
```
...
haupttabelle_df %>% 
  ...
  geom_errorbar(mapping = aes(ymin = ci_lwr, ymax = ci_upr), color = "dark red", alpha = 0.5) +
  labs(title = "Share of Women Grades", y = "Share")
```
...
haupttabelle_df %>% 
  ...
  labs(title = "Success Ratio per Starting Year (Studienbeginn)", y = "Success Rate")
```
